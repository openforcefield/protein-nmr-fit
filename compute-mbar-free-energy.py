...
def main(
    ...
    for window_index, window in enumerate(windows):
        ...
        for replica in replicas:
            ...
            window_fraction_native_contacts = numpy.loadtxt(
                window_cv_path,
                usecols=1,
                max_rows=length,
            )
            ...
            if end_length != None:
                window_fraction_native_contacts = (
                    window_fraction_native_contacts[-end_length:]
                )

            N_window_samples += len(window_fraction_native_contacts)
            fraction_native_contacts.extend(window_fraction_native_contacts)
            sample_replicas.extend([replica] * len(window_fraction_native_contacts))
            ...
            uncorrelated_fraction_native_contacts.extend(
                window_fraction_native_contacts[window_uncorrelated_sample_indices]
            )
    ...
    if N_windows == 11:
        out_prefix = f"{out_prefix}-0.8"
    elif N_windows == 16:
        out_prefix = f"{out_prefix}-0.7"
