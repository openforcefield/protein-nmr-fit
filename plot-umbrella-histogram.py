...
def _plot_histogram(
    ...
    for row_index, row_label in enumerate(row_labels):
        ...
        for category_index, category_label in enumerate(category_labels):
            ...
            if shade_bins:
                bins = numpy.linspace(0.38, 1.0, 32)
                for i in numpy.arange(1, 30, 2):
                    pyplot.axvspan(bins[i], bins[i + 1], color="gray", alpha=0.5)

    if x_ticks is None:
        x_ticks = numpy.round(
            numpy.arange(
                numpy.floor(min_x / 0.05) * 0.05,
                numpy.ceil(max_x / 0.05) * 0.05 + 0.025,
                0.05,
            ),
            1,
        )
...
def main(
    ...
    for output_prefix in [
        ...
        if output_prefix == "ff14sb-opc3":
            ...
            ff_labels = {
                f"ff14SB-{wm_prefix.upper()}": output_prefix,
            }

        elif output_prefix == "null-0.0.3-pair-opc3":
        ...
        elif output_prefix == "specific-0.0.3-sage-pair-opc3":
            wm_prefix = output_prefix.split("-", maxsplit=2)[2]

            ff_labels = {
                "Specific-0.0.3-Sage-Pair-{wm_prefix.upper()}": output_prefix,
            }
                ...
                for replica in replicas:
                    for window in windows:
                        ...
                        window_df = pandas.read_csv(
                            ...
                            sep="\s+",
                            skiprows=1,
                            names=["Step", "Fraction native contacts"],
                        )
