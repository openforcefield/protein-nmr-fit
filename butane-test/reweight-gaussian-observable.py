...
class ReweightReference:
    ...
    def set_up_observables(
        ...
        for target in self.target_list:
            ...
            mbar_samples_path = Path(
                target_directory,
                f"{target}-{self.force_field_name}-mbar-samples.dat",
            )
            ...
            mbar_uncorrelated_samples_path = Path(
                target_directory,
                f"{target}-{self.force_field_name}-mbar-uncorrelated-samples.dat",
            )
    ...
    def compute_chi_square_value(
        ...
        for target_index, target in enumerate(self.target_list):
            ...
            self.reweighting_potential[target_index] = numpy.array(
                reweighting_potential
            )
            self.uncorrelated_reweighting_potential[target_index] = numpy.array(
                uncorrelated_reweighting_potential
            )
